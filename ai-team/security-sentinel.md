...
> **Arquitectura de referencia:** cuando el perfil activo es `next-tailwind`, verifica también contra **`.cursor/skills/frontend/next_architecture.md`** — validar que no haya SQL inline en `app/`, que las API routes delegen en `services/`, y que la gestión de sesiones use `lib/auth.js`.
